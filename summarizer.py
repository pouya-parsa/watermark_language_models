...
def summarize(text, watermark=False, gamma=0.25, delta=2.0):
  if not watermark:
    ...
    summary = model.generate(**inputs, return_dict_in_generate=True, max_new_tokens=1000, output_scores=True, num_beams=1, do_sample=True)
    ...
    output_tokens = model.generate(**tokenized_input,
                                  logits_processor=LogitsProcessorList([watermark_processor]), max_new_tokens=1000, return_dict_in_generate=True, output_scores=True, num_beams=1, do_sample=True)
